...
const SavedPosts: React.FC = () => {
	...
	return (
		<div className="mx-auto w-full max-w-7xl rounded-2xl border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/35 to-sky-50/35 px-2 py-4 md:px-4 md:py-6">
			<div className="mb-6 rounded-2xl border border-emerald-100 bg-white/95 p-4 shadow-sm md:p-6">
			...
			{loading ? (
				<div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
					{Array.from({ length: 8 }).map((_, index) => (
						<div
							key={index}
							className="h-72 animate-pulse rounded-xl bg-emerald-100/70"
						/>
					))}
				</div>
			) : savedPosts.length === 0 ? (
				<div className="rounded-2xl border border-emerald-100 bg-white p-8 text-center text-slate-600">
